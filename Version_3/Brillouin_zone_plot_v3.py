...
def add_b_vector(fig, bvec, name, color):
    origin = np.zeros(3)
...
def make_bz_plot(
    ...
    # BZ faces and black edges, close to the pymatviz-looking aesthetic.
    for face in faces:
    ...
    if show_b_vectors:
        ...
        for idx, bvec in enumerate(bz_reciprocal_lattice, start=1):
            add_b_vector(fig, bvec, f"b{idx}", b_colors[idx - 1])
    ...
    for seq in path_sequences:
        ...
        for label in seq:
            cart = point_cart_coords[label]
            coords.append(np.array(cart, dtype=float))
...
def run_structure_mode(args):
    ...
    make_bz_plot(
        ...
        show_b_vectors=not args.hide_b_vectors,
    )
...
def main():
    ...
    parser.add_argument(
        "--interactive",
        action="store_true",
        help="Ask whether to use recommended path or enter a fully manual path.",
    )
    ...
    parser.add_argument(
        "--hide-b-vectors",
        action="store_true",
        help="Hide reciprocal basis vectors b1, b2, b3.",
    )
